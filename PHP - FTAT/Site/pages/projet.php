...
if (isset($bdd)) {
    $sql = "SELECT * FROM AgileTools.projets WHERE IdP = :id_projet";
...
<div class="projet">
    ...
    <div class="projet-info">
                                ...
                                <td>
                                    ...
                                    if (!empty($tasks)){
                                        ...
                                        <?php 
                                            ...
                                            $teams = getEquipeFromUserInProject($membre['IdU'], $projet['IdP']);

                                            $sprint_a_teams = [];
                                            if ($teams) {
                                                foreach ($teams as $team) {
                                                    $active_sprint = getActiveSprintOfTeam($team['IdEq']);
                                                    if (getActiveSprintOfTeam($team['IdEq']) != null) {
                                                        $sprint_a_teams[] = [$active_sprint['IdS'], $team['NomEqPrj']];
                                                    }
                                                }
                                            }

                                        ?>
                                        <?php if ($tasks && $sprint_a_teams): ?>
                                        <form action="./process/assign_task_process.php" method="POST">
                                            ...
                                            <div class="form-group">
                                                <select name="id_sprint" id="id_sprint" required>
                                                    <option value="">-- Sélectionner une équipe --</option>
                                                    <?php foreach($sprint_a_teams as $tuple):?>
                                                        <option value="<?php echo $tuple[0]; ?>"><?php print_r($tuple[1]) ?></option>
                                                    <?php endforeach; ?>
                                                </select>
                                            </div>
        ...
        <?php if (isset($is_scrum_master) && $is_scrum_master):
            $sql = "SELECT IdU, NomU, PrenomU FROM AgileTools.utilisateurs";
            $stmt = $bdd->query($sql);
        ...
        <div class="form-add-tache-projet">
            ...
            <form action="./process/add_task_process.php" method="POST">
                ...
                <select name="task_priority" id="task_priority" required>
                    ...
                    <?php
                    $sql = "SELECT IdPriorite, Priorite FROM AgileTools.prioritestaches";
                    $stmt = $bdd->query($sql);
            ...
            <?php if (isset($equipes) && isInATeamInProjet($_SESSION['user_id'],$id_projet)): ?>
                <div class="form-add-bac-projet">
                    <form id="ideeForm" action="./process/add_idee_process.php" method="POST">
                        ...
                        <div class="form-group">
                            <select name="bac_IdEq" id="bac_IdEq" required>
                                <option value="">-- Sélectionner une équipe --</option>
                                <?php foreach(getEquipeFromUserInProject($_SESSION['user_id'], $id_projet) as $team):?>
                                    <option value="<?php echo $team['IdEq']; ?>"><?php echo $team['NomEqPrj']; ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    ...
                    <?php if (isset($is_product_owner) && ($is_product_owner)): ?>
